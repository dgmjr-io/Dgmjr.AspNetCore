...
public class DirectoryObjectsService(
    GraphServiceClient graph,
    ILogger<UsersService> logger,
    IOptionsMonitor<AzureAdB2CGraphOptions> options,
    IOptionsMonitor<MicrosoftIdentityOptions> msidOptions,
    IDistributedCache cache
) : MsGraphService(graph, logger, options, msidOptions, cache), IDirectoryObjectsService
{
    public async Task<DirectoryObject> GetAsync(
        string id,
    ...
    {
        return await Graph.DirectoryObjects[id]
            .Request()
            .Select(property)
