...
public static class JwtExtensions
{
            ...
            .AddJwtBearer(opts =>
            {
                ...
                opts.TokenValidationParameters = new TokenValidationParameters
                {
                    ValidIssuer = builder.Configuration[
                        $"{nameof(JwtConfigurationOptions)}:{nameof(JwtConfigurationOptions.ClaimsIssuer)}"
                    ],
    ...
    public static WebApplication UseJwtBearerAuthentication(this WebApplication app)
    {
        _ = app.MapGet(
            ...
            async context =>
            {
                var claims = new[] { new C(Ct.Name, "dgmjr"), new C(Ct.Role, "admin") };
                ...
                var token = new JwtSecurityToken(
                    issuer: app.Configuration[
                        $"{nameof(JwtConfigurationOptions)}:{nameof(JwtConfigurationOptions.Issuer)}"
                    ],
                    audience: app.Configuration[
                        $"{nameof(JwtConfigurationOptions)}:{nameof(JwtConfigurationOptions.Audience)}"
                    ],
                    claims: claims,
